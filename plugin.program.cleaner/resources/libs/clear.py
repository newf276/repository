...
def clear_crash():
    ...
    if len(files) > 0:
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            ...
                            nolabel="[B][COLOR deepskyblue]Keep Logs[/COLOR][/B]"):
            for f in files:
                os.remove(f)
...
def total_clean():
    ...
    if dialog.yesno(CONFIG.ADDONTITLE,
                        ...
                        nolabel='[B][COLOR deepskyblue]Cancel Process[/COLOR][/B]',
...
def clear_thumbs(type=None):
    ...
    if type is not None:
        ...
        choice = dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Would you like to delete the {1} and related thumbnail folders?'.format(CONFIG.COLOR2, latest) + '\n' + "They will repopulate on the next startup[/COLOR]", nolabel='[B][COLOR deepskyblue]Don\'t Delete[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Delete Thumbs[/COLOR][/B]')
    if choice == 1:
        try:
...
def remove_addon(addon, name, over=False, data=True):
    ...
    if over:
        ...
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                               ...
                               nolabel='[B][COLOR deepskyblue]Don\'t Remove[/COLOR][/B]')
    if yes == 1:
...
def remove_addon_data(addon):
    ...
    if addon == 'all':  # clear ALL addon data
        ...
        if addon in CONFIG.EXCLUDES:
            ...
            if dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Would you also like to remove the add-on data for:[/COLOR]'.format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, addon), yeslabel='[B][COLOR springgreen]Remove Data[/COLOR][/B]', nolabel='[B][COLOR deepskyblue]Don\'t Remove[/COLOR][/B]'):
