...
def highlight_text(msg):
    ...
    for item in matches:
        ...
        msg = msg.replace(string, '[COLOR dodgerblue]{0}[/COLOR]'.format(string))
    msg = msg.replace('WARNING', '[COLOR yellow]WARNING[/COLOR]').replace('ERROR', '[COLOR dodgerblue]ERROR[/COLOR]').replace('[NL]', '\n').replace(': EXCEPTION Thrown (PythonToCppException) :', '[COLOR dodgerblue]: EXCEPTION Thrown (PythonToCppException) :[/COLOR]')
