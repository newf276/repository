...
class MetaCache:
	...
	def get_memory_cache_season(self, prop_string, current_time):
		try:
			cachedata = eval(get_property('finder.meta_season_%s' % prop_string))
			if cachedata[0] > current_time: result = cachedata[1]
		except: result = None
		return result
	...
	def set_memory_cache_season(self, prop_string, meta, expires):
		try:
			...
			set_property('finder.meta_season_%s' % prop_string, repr(cachedata))
		except: pass

	def delete_memory_cache(self, media_type, id_type, media_id):
		try: clear_property('finder.%s_%s_%s' % (media_type, id_type, media_id))
		except: pass

	def delete_memory_cache_season(self, prop_string):
		try: clear_property('finder.meta_season_%s' % prop_string)
		except: pass
