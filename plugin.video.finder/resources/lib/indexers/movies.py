...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external: kodi_utils.set_property('finder.exit_params', kodi_utils.folder_path())
			...
			items = self.worker()
			if self.list and not items: logger('Finder', 'movies.fetch_list empty build for %s (%s ids, external=%s)' % (self.action, len(self.list), self.is_external))
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			set_properties({
				...
				'finder.browse_movie_set_params': browse_movie_set_params,
				...
				'finder.browse_similar_params': browse_similar_params,
