		...
		if data:
			if shuffle_lists:
				...
				if returning_to_list:
					try: data = json.loads(kodi_utils.get_property('finder.personal.lists.order'))
					except: pass
				else:
					shuffle(data)
...
def make_new_personal_list(params):
	...
	if not external_creation and not is_retry and kodi_utils.confirm_dialog(
		...
		if suggested_author in ('Collection', 'Watchlist'): suggested_author = get_setting('finder.trakt.user')
