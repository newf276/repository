...
def refresh_widgets():
	...
	if get_setting('finder.widget_refresh_notification', 'true') == 'true': kodi_utils.notification('Widgets Refreshed', 2500)
...
class RandomLists():
	...
	def random_because_you_watched(self):
		...
		try:
			...
			self.list_items = self.function(self.params).worker()
			self.category_name =  list_name
		except: kodi_utils.clear_property('finder.random_because_you_watched')
		self.make_directory()
	...
	def make_directory(self, next_page_params={}):
		...
		if self.is_external:
			if self.folder_name: kodi_utils.set_property('finder.%s' % self.folder_name, self.category_name)
			else: kodi_utils.set_property('finder.%s' % self.base_list_name, self.category_name)
...
def random_shortcut_folders(folder_name, random_results):
	...
	if is_external: kodi_utils.set_property('finder.%s' % folder_name, list_name)
	if menu_type == 'movie':
