...
def addon_icon_mini():
	return get_property('finder.addon_icon_mini') or os.path.join(addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
...
def get_icon(image_name, image_folder='icons'):
	return 'https://raw.githubusercontent.com/%s/%s/master/packages/media/%s/%s.png' \
			% (get_property('finder.update.username'), get_property('finder.update.location'), image_folder, image_name)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('finder.use_viewtypes') == 'true': return
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('finder.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
