...
def get_external_cache_status(debrid, unchecked_hashes, data, active_debrid):
	...
	try:
		results = []
		imdb_id = data['imdb']
		debrid_name, services, token = {'Real-Debrid': ('realdebrid', ['torrentio'], get_setting('finder.rd.token')),
										'AllDebrid': ('alldebrid', ['mediafusion'], get_setting('finder.ad.token'))}[debrid]
