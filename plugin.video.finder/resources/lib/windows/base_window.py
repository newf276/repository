...
def window_manager(obj):
	...
	def get_stack():
		try: window_stack = json.loads(kodi_utils.get_property('finder.window_stack'))
		except: window_stack = []
		return window_stack
...
class FontUtils:
	def execute_custom_fonts(self):
		...
		skin_files = kodi_utils.list_dirs(kodi_utils.translate_path('special://home/addons/plugin.video.finder/resources/skins/Default/1080i/'))[1]
	...
	def skin_change_check(self):
		...
		if self.current_skin != kodi_utils.get_property('finder.current_skin') or self.current_font != kodi_utils.get_property('finder.current_font'): return True
		return False
