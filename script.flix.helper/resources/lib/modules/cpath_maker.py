...
(
    ...
    "script-flix-widget_movies",
    "script-flix-widget_tvshows",
    "script-flix-widget_custom1",
    "script-flix-widget_custom2",
    "script-flix-widget_custom3",
)
(
    ...
    "script-flix-main_menu_movies",
    "script-flix-main_menu_tvshows",
    "script-flix-main_menu_custom1",
    "script-flix-main_menu_custom2",
    "script-flix-main_menu_custom3",
)
default_xmls = {
    ...
    def manage_action(self, cpath_setting, context="widget"):
        ...
        if action in ["move_up", "move_down"]:
            ...
            if len(parts) < 3 or not parts[-1].isdigit():
                dialog.ok("Flix", "Cannot move this widget")
                return None
            ...
            if result:
                ...
                if context == "widget":
                    ...
                    self.make_main_menu_xml(self.fetch_current_cpaths())
                    dialog.ok("Flix", "Main menu remade")
...
def remake_all_cpaths(silent=False):
    ...
    if not silent:
        xbmcgui.Dialog().ok("Flix", "Menus and widgets remade")
    ...
    for item in (
        ...
        try:
            ...
            for count in range(1, 51):
                ...
                window.setProperty("flix.%s.label" % list_id, cpath_label)
                window.setProperty("flix.%s.path" % list_id, cpath_path)
        except:
            pass
    try:
        del window
...
def starting_search_widgets():
    ...
    window.setProperty("flix.search.widgets", "finished")
    search_widgets = {
        ...
        try:
            ...
            try:
                ...
                if not first_item:
                    continue
                cpath_label, cpath_path = first_item["label"], first_item["file"]
                window.setProperty(f"flix.{widget_id}.label", cpath_label)
                window.setProperty(f"flix.{widget_id}.path", cpath_path)
            except:
                continue
        except:
            pass
...
# def starting_widgets():
#     window = xbmcgui.Window(10000)
#     window.setProperty("flix.starting_widgets", "finished")
#     for item in (
#         "movie.widget",
#         "tvshow.widget",
#         "custom1.widget",
