...
def show_changelog():
    helper_addon = xbmcaddon.Addon("script.flix.helper")
    helper_version = helper_addon.getAddonInfo("version")
    skin_addon = xbmcaddon.Addon("skin.flix")
    skin_version = skin_addon.getAddonInfo("version")
    skin_name = skin_addon.getAddonInfo("name")
    changelog_path = xbmcvfs.translatePath("special://skin/flixchangelog.txt")
    ...
    helper_pattern = r"(Flix Helper: Latest: v([\d.]+)) \| Installed: v.*"
    ...
    if helper_match:
        ...
        if helper_version != latest_helper_version:
            ...
            update_message = (
                "\n\n[COLOR red][B]An update is available for the Flix Helper. "
                ...
                "\n2. Find Flix Helper and click it"
                "\n3. Versions [B]»[/B] Click the latest version in ivarbrandt's Repository"
            )
        else:
            update_message = "\n\n[COLOR limegreen][B]Flix Helper up to date. No updates needed at this time.[/B][/COLOR]"
...
def set_image():
    ...
    if image_file and image_file != last_path:
        ...
        xbmc.executebuiltin(f"Skin.SetString(FlixCustomBackground,{image_file})")
