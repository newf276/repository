...
class RatingsDatabase:
    ...
    #     dialog.ok("Flix", "All ratings have been cleared from the database.")

    # def delete_all_ratings(self):
    #     dialog = xbmcgui.Dialog()
    #     if dialog.yesno("Flix", "Are you sure you want to clear all ratings?"):
    #         try:
    #             with sqlite3.connect(self.db_path, timeout=60) as conn:
    #                 cursor = conn.cursor()
    #                 cursor.execute("DELETE FROM ratings")
    ...
    #             dialog.notification("Flix", "Error clearing ratings", xbmcgui.NOTIFICATION_ERROR)

    def delete_all_ratings(self, silent=False):
        if not silent:
            dialog = xbmcgui.Dialog()
            if not dialog.yesno("Flix", "Are you sure you want to clear all ratings?"):
                return
        try:
            with sqlite3.connect(self.db_path, timeout=60) as conn:
                cursor = conn.cursor()
