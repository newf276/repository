...
SEARCH_DATABASE_PATH = xbmcvfs.translatePath(
    "special://profile/addon_data/script.flix.helper/spath_cache.db"
)

search_history_xml = "script-flix-search_history"
...
class SPaths:
    ...
    def remove_all_spaths(self, skip_dialog=False):
        count_str = self.home_window.getProperty("flix.search.history.count")
        ...
        for i in range(1, 101):
            self.home_window.clearProperty(f"flix.search.history.{i}")
            self.home_window.clearProperty(f"flix.search.history.{i}.id")
        self.home_window.setProperty("flix.search.history.count", "0")
        self.home_window.clearProperty("flix.search.input")
        self.home_window.clearProperty("flix.search.input.encoded")
        self.home_window.clearProperty("flix.search.input.trakt.encoded")
        self.home_window.setProperty(
            "flix.search.history.empty",
    ...
    def refresh_search_history(self):
        ...
        for i, (id, term) in enumerate(history[: self.max_history_items], 1):
            self.home_window.setProperty(f"flix.search.history.{i}", term)
            self.home_window.setProperty(f"flix.search.history.{i}.id", str(id))
        count = min(len(history), self.max_history_items)
        self.home_window.setProperty("flix.search.history.count", str(count))
        if count == 0:
            self.home_window.setProperty(
                "flix.search.history.empty",
            ...
            self.home_window.clearProperty("flix.search.history.empty")
    ...
    def update_search_history_properties(self, search_term, existing_spath_id):
        ...
        for i in range(1, count + 1):
            if self.home_window.getProperty(f"flix.search.history.{i}") == search_term:
                existing_property_index = i
                break
        if existing_property_index is not None:
            term_to_move = self.home_window.getProperty(
                f"flix.search.history.{existing_property_index}"
            )
            id_to_move = self.home_window.getProperty(
                f"flix.search.history.{existing_property_index}.id"
            )
            for i in range(existing_property_index, 1, -1):
                prev_term = self.home_window.getProperty(f"flix.search.history.{i-1}")
                prev_id = self.home_window.getProperty(f"flix.search.history.{i-1}.id")
                self.home_window.setProperty(f"flix.search.history.{i}", prev_term)
                self.home_window.setProperty(f"flix.search.history.{i}.id", prev_id)
            self.home_window.setProperty("flix.search.history.1", term_to_move)
            self.home_window.setProperty("flix.search.history.1.id", id_to_move)
        else:
            for i in range(min(count, self.max_history_items - 1), 0, -1):
                term = self.home_window.getProperty(f"flix.search.history.{i}")
                term_id = self.home_window.getProperty(f"flix.search.history.{i}.id")
                self.home_window.setProperty(f"flix.search.history.{i+1}", term)
                self.home_window.setProperty(f"flix.search.history.{i+1}.id", term_id)
            self.home_window.setProperty("flix.search.history.1", search_term)
            self.home_window.setProperty(
                "flix.search.history.1.id", str(existing_spath_id)
            )
            if count < self.max_history_items:
                self.home_window.setProperty(
                    "flix.search.history.count", str(count + 1)
                )
    ...
    def search_input(self, search_term=None, from_history=False):
        if search_term is None or not search_term.strip():
            ...
            if keyboard.isConfirmed():
                ...
                return
        self.home_window.setProperty("flix.search.refreshing", "true")
        ...
        self.home_window.setProperty("flix.search.input", search_term)
        self.home_window.setProperty("flix.search.input.encoded", encoded_search_term)
        self.home_window.setProperty(
            "flix.search.input.trakt.encoded", encoded_search_term
        )
        xbmc.sleep(200)
        if not from_history:
        ...
        def load_widgets_and_clear_flag():
            ...
            xbmc.sleep(800)
            self.home_window.clearProperty("flix.search.refreshing")
...
#     if not silent:
#         xbmcgui.Dialog().ok("Flix", "Search history remade")
