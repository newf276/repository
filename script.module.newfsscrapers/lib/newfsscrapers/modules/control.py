...
addonObject = addon('script.module.newfsscrapers')
...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('newfsscrapers_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('newfsscrapers_settings', jsdumps(settings_dict))
		return settings_dict
	except:
		return None

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear newfsscrapers.log if setting to reverse has been changed
	if homeWindow.getProperty('newfsscrapers.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('newfsscrapers.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://script.module.newfsscrapers/?action=tools_clearLogFile)')
...
def isVersionUpdate():
	...
	try:
		...
		xbmc.log('newfsscrapers Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
		addon_folder = transPath('special://profile/addon_data/script.module.newfsscrapers')
		xbmcvfs.mkdirs(addon_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = addon('script.module.newfsscrapers').getAddonInfo('version')
...
def checkPackages():
	...
	for item in files:
		if '.newfsscrapers' in item and item.endswith('zip'):
...
def clean_settings():
	def _make_content(dict_object):
		...
		content += '\n</settings>'
		return content
	addon_id = 'script.module.newfsscrapers'
	try:
		removed_settings = []
		active_settings = []
...
def notification(title=None, message=None, icon=None, time=3000, sound=False):
	...
	if icon in (None, '', 'default'): icon = joinPath(addonInfo('path'), 'resources', 'icon.png')
