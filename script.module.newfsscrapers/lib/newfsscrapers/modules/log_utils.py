...
import inspect
from newfsscrapers.modules.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
DEBUGPREFIX = '[COLOR red][ newfsscrapers %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'newfsscrapers.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ script.module.newfsscrapers ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		if exception:
			...
			addon = 'script.module.newfsscrapers'
		...
		import xbmc
		xbmc.log('[ script.module.newfsscrapers ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)

def clear_logFile():
	cleared = False
	try:
		from newfsscrapers.modules.control import yesnoDialog
		...
		import xbmc
		xbmc.log('[ script.module.newfsscrapers ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def view_LogFile(name):
	try:
		...
		from newfsscrapers.modules.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from newfsscrapers.modules.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from newfsscrapers.modules.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		UserAgent = 'script.kodi.loguploader: 1.0' #'newfsscrapers %s' % addonVersion()
		...
		if 'key' in response.json():
			...
			log('newfsscrapers log file uploaded to: %s' % result)
			...
			select = selectDialog([i[0] for i in list], 'newfsscrapers log file uploaded to:')
			if 'Copy url To Clipboard' in list[select][0]:
				from newfsscrapers.modules.source_utils import copy2clip
				copy2clip(list[select - 1][1])
		elif 'message' in response.json():
			notification(message='newfsscrapers Log upload failed: %s' % str(response.json()['message']))
			log('newfsscrapers Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
		else:
			notification(message='newfsscrapers Log upload failed')
			log('newfsscrapers Log upload failed: %s' % response.text, level=LOGERROR)
	except:
		error('newfsscrapers log upload failed')
